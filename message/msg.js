          ...
          try {
                ...
                async function sendPlay(from, query) {
                  try {
                    ...
                    var button = [
                        { buttonId: prefix+`ytmp3 ${data.url}`, buttonText: { displayText: `MUSIK` }, type: 1 },
                        { buttonId: prefix+`ytmp4 ${data.url}`, buttonText: { displayText: `VIDIO` }, type: 1 }
                    ]
                    conn.sendMessage(from, { caption: `*Judul :* ${data.title}\n*Kualitas Mp3 :* 128p\n*Kualitas Mp4 :* 360p\n*Durasi :* ${data.duration.timestamp}\n*Tautan :* ${data.url}`, image: await getBuffer(data.thumbnail), buttons: button, footer: 'Pilih Salah satu untuk mengunduh', mentions: [sender], viewOnce: true }, { quoted: msg })
                  } catch (e) {
                ...
                const buttonsDefault = [
                   { urlButton: { displayText: `Follow Ig Owner`, url: setting.youtube } }
                ]
